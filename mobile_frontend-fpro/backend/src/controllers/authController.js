...
const getProfile = async (req, res) => {
  try {
    ...
    // [Changement] Inclure les compétences pour les techniciens
    const userRole = await User.findByPk(req.userId).then(u => u?.role);
    let ratingStats = { averageRating: 0, reviewCount: 0 };

    if (userRole === 'technicien') {
      const { Technician, Review } = require('../models');
      include.push({
        model: Technician,
        as: 'technician_profile',
        attributes: ['skills', 'is_available']
      });

      // [Ajout] Calculer les statistiques d'avis
      const stats = await Review.findOne({
        where: { technician_id: req.userId },
        attributes: [
      ...
      ratingStats.averageRating = parseFloat(stats.averageRating || 0).toFixed(1);
      ratingStats.reviewCount = parseInt(stats.reviewCount || 0);
    }
    ...
    // [Changement] Aplatir les compétences et ajouter les avis dans la réponse pour le technicien
    if (user.role === 'technicien') {
      if (user.technician_profile) {
        responseData.skills = user.technician_profile.skills;
      }
      responseData.averageRating = ratingStats.averageRating;
      responseData.reviewCount = ratingStats.reviewCount;
    }

    return ResponseHandler.success(res, responseData, 'Profil récupéré');
...
const updateProfile = async (req, res) => {
  try {
    const allUpdates = req.validatedData;
    ...
    // [Changement] Filtrage des champs modifiables selon le rôle
    let allowedUpdates = {};
    if (user.role === 'admin') {
      // Admin peut tout modifier
      allowedUpdates = {
        first_name: allUpdates.first_name,
        last_name: allUpdates.last_name,
        email: allUpdates.email,
        phone: allUpdates.phone
      };
    } else if (user.role === 'agent') {
      // Agent peut modifier uniquement le téléphone
      allowedUpdates = {
        phone: allUpdates.phone
      };
    } else if (user.role === 'technicien') {
      // Technicien peut modifier téléphone et skills
      allowedUpdates = {
        phone: allUpdates.phone
      };

      // Gérer les compétences séparément
      if (allUpdates.skills) {
        const { Technician } = require('../models');
        await Technician.update(
          { skills: allUpdates.skills },
    ...
    // Vérifier si le nouvel email n'est pas déjà utilisé (si admin tente de changer son email)
    if (allowedUpdates.email && allowedUpdates.email !== user.email) {
      const existingUser = await User.findOne({ where: { email: allowedUpdates.email } });
      if (existingUser) {
        return ResponseHandler.error(res, 'Cet email est déjà utilisé', 409);
      }
    }
